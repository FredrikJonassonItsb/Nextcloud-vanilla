...
class GuestController extends AEnvironmentAwareOCSController {
	...
	public function __construct(
		...
		private ISession $session, // itsl change
	) {
	...
	public function setDisplayName(string $displayName): DataResponse {
		...
		// itsl change start
		if ($this->session->exists('BankIdAuthUserName')) {
			$this->guestManager->updateName($this->getRoom(), $participant, $this->session->get('BankIdAuthUserName'));
			return new DataResponse($this->session->get('BankIdAuthUserName'));
		}
		// itsl change end
