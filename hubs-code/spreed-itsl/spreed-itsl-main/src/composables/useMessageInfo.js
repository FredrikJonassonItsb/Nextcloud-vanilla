...
export function useMessageInfo(message = ref({})) {
	...
	const actorDisplayName = computed(() => {
		const token = message.value.token
		const actorId = message.value.actorId
		const msgName = (message.value.actorDisplayName || '').trim()
		...
		itsl change end. */
		// non guest actor types
		if ([ATTENDEE.ACTOR_TYPE.GUESTS, ATTENDEE.ACTOR_TYPE.EMAILS].includes(message.value.actorType)) {
			return msgName;
		}

		// first attemp using guestNameStore
		const guestNameStore = useGuestNameStore()
		const fromGuestStore = guestNameStore.getGuestName(token, actorId)
		if (fromGuestStore && fromGuestStore !== t('spreed', 'Guest')) {
			return fromGuestStore
		}

		// second attemp using participants store
		const participants = (store.getters.participants && store.getters.participants(token)) || []
		const p = participants.find(p => p.actorId === actorId)
		const fromParticipants = (p?.displayName || '').trim()
		if (fromParticipants && fromParticipants !== t('spreed', 'Guest')) {
			guestNameStore.addGuestName({
				token,
				actorId,
				actorDisplayName: fromParticipants,
			}, { noUpdate: false })
			return fromParticipants
		}

		// third attemp - message payload
		if (msgName && msgName !== t('spreed', 'Guest')) {
			guestNameStore.addGuestName({
				token,
				actorId,
				actorDisplayName: msgName,
			}, { noUpdate: false })
			return msgName
		}
