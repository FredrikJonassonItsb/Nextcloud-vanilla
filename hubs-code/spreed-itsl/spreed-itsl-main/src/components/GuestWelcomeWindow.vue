...
import { ref } from 'vue'
itsl change end */
import { ref, onMounted } from 'vue' // itsl change
...
import { loadState } from '@nextcloud/initial-state' // itsl change
...
export default {
	...
	setup(props) { // itsl change
		...
		// itsl change start
		onMounted(async () => {
			const participantData = loadState('spreed', 'participantDisplayName', null)
			console.debug("CURRENT USER IS GUEST, SETTING guest name from GuestWelcomeWindow", participantData)
			if (participantData) {
				console.debug("calling submit guestusername")
				await guestNameStore.submitGuestUsername(props.token, participantData)
			}
			// NOTE: consider redirecting to error page since we do not allow guests without participantDisplayName being set at this stage
		})
		// itsl change end
		return {
			guestNameStore,
