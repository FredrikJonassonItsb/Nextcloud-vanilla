	...
	<NcListItem :name="computedName"
		...
		<template v-if="showParticipantActions" #actions>
			...
			<template v-if="showPermissionsOptions">
				...
				<!-- itsl change start -->
				<NcActionButton key="view-full-identity"
					close-after-click
					@click="showFullGuestIdentity">
					<template #icon>
						<Eye :size="20" />
					</template>
					{{ t('spreed', 'View verified identity (moderator only)') }}
				</NcActionButton>
				<!-- itsl change stop -->
			</template>
...
import axios from '@nextcloud/axios' // itsl
import { generateUrl } from '@nextcloud/router' // itsl
import Eye from 'vue-material-design-icons/Eye.vue' // itsl
...
export default {
	...
	methods: {
		...
		// itsl change start
		async showFullGuestIdentity() {
			try {
				const url = generateUrl(
					'/apps/sdkmc/api/v2/spreed/guest-identity/{token}/{actorId}',
					{
						token: this.token,
						actorId: this.participant.actorId,
					}
				);
				...
				if (data?.success) {
					...
					showError(t('spreed', 'No additional info for this attendee'));
				}
			} catch (error) {
				showError(t('spreed', 'No additional info for this attendee'));
			}
		...
		showIdentityModal(identity) {
			showSuccess(t('spreed', 'Guest: {firstName} {lastName} (SSN: {ssn})', {
